...
const resolveAuthenticatedUser = async (authHeader?: string) => {
...
const attachOptionalAuth = async (
  req: Request,
  _res: Response,
  next: NextFunction,
) => {
...
// Helper calculation functions exported for testability
...
app.get("/api/stats/streak", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
app.get("/api/stats/heatmap", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
app.get("/api/stats/weak-topic", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const topics = await prisma.topic.findMany({
      include: {
...
app.get("/api/stats/topic-breakdown", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const topics = await prisma.topic.findMany({
      include: {
...
app.get("/api/stats/difficulty-by-month", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const solves = await prisma.progress.findMany({
      where: { userId, status: "DONE" },
...
const normalizeLearningObjectives = (value: unknown): string[] => {
...
app.post(
  "/api/admin/learn/seed-comprehensive",
  requireAuth,
  requireAdmin,
  async (_req: Request, res: Response) => {
    try {
