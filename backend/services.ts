...
export const getTimeAnalytics = async (userId: string) => {
    ...
    const weeklyTrends = weeks.map(w => {
        ...
        const avg = (arr: typeof weekProblems) => arr.length > 0 ? Math.round(arr.reduce((s, p) => s + p.timeSpent, 0) / arr.length) : 0;
    ...
    const speedInsights: Array<{ difficulty: string; recentAvg: number; olderAvg: number; change: number }> = [];
    ...
    // === Topic breakdown ===
    const topicMap: Record<string, { name: string; totalTime: number; count: number }> = {};
    allProgress.forEach(p => {
...
export const getAchievements = async (userId: string) => {
    ...
    const touchedTopics = topics.filter(t =>
        t.problems.some(p => solvedIds.has(p.id))
    );
...
export const getWeeklyReport = async (userId: string) => {
    ...
    // This week's solves
    const thisWeekSolves = solvedAll.filter(p =>
        p.completedAt && new Date(p.completedAt) >= weekAgo
    );
    // Last week's solves
    const lastWeekSolves = solvedAll.filter(p =>
        p.completedAt && new Date(p.completedAt) >= twoWeeksAgo && new Date(p.completedAt) < weekAgo
    );
