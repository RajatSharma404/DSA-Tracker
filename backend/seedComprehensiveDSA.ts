...
import { generateTopicContent } from "./allTopicsTutoring";
...
export async function seedComprehensiveDSA(): Promise<SeedResult> {
  try {
    ...
    for (const topic of TOPICS_SEQUENCE) {
      ...
      const theoryContent = generateTopicContent(topic.idx);

      const theoryBlockId = randomUUID();
