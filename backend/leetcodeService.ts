...
export const submitCodeToLeetCode = async (
  ...
  try {
    ...
    let lastFailure: { status: number; data: any; questionId: string } | null =
      null;
    ...
    for (const questionId of questionIds) {
      ...
      console.error(
        `LeetCode Submission Failed (${response.status}) using question_id=${questionId}:`,
        JSON.stringify(response.data, null, 2),
      );
    }

    throw new Error(
      `LeetCode rejected submission for all question IDs: ${JSON.stringify(lastFailure)}`,
    );
  } catch (error: any) {
