...
async function main() {
    ...
    const freeProblems = pairs.filter((p) => !p.paid_only && !p.stat.question__hide);
    ...
    const getDifficulty = (level) => {
        switch (level) {
            case 1: return client_1.Difficulty.EASY;
            case 2: return client_1.Difficulty.MEDIUM;
            case 3: return client_1.Difficulty.HARD;
            default: return client_1.Difficulty.EASY;
        }
    };
    console.log('Formatting data for Prisma...');
    const prismaProblems = problemsToInsert.map((p, index) => {
        return {
...
//# sourceMappingURL=seed-leetcode.js.map