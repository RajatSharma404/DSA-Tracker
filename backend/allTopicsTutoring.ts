...
const TOPIC_CONTENTS: Record<number, TopicContent> = {
  1: {
    ...
    whatIsIt:
      "Complexity analysis tells us how runtime and memory grow as input grows. It is the first filter for deciding whether a solution will survive interview constraints.",
    concept: `
...
Answer: about 100 times slower.`,
  ...
  2: {
    title: "Arrays",
    whatIsIt:
      "An array stores same-type elements in contiguous memory. That makes random access instant, but inserting in the middle expensive because elements must shift.",
    concept: `
Index:  0   1   2   3   4
...
Answer: because every element to the right must shift.`,
  ...
  3: {
    title: "Strings",
    whatIsIt:
      "A string is an ordered sequence of characters. In DSA, strings are usually solved with two pointers, hashing, or sliding windows.",
    concept: `
String:  h  e  l  l  o
Index:   0  1  2  3  4
...
Answer: fixed-size arrays avoid hashing overhead.`,
  ...
  4: {
    title: "Linked Lists",
    whatIsIt:
      "A linked list stores nodes that point to the next node. It is flexible for insertions and deletions, but you cannot jump directly to an index like in an array.",
    concept: `
head -> [1|*] -> [2|*] -> [3|null]
...
Answer: you must follow pointers one node at a time.`,
  ...
  5: {
    title: "Stack & Queue",
    whatIsIt:
      "A stack is LIFO and a queue is FIFO. They model real processes like undo stacks, browser history, ticket lines, and BFS traversal.",
    concept: `
...
Answer: stack is LIFO, queue is FIFO.`,
  ...
  6: {
    title: "Hashing",
    whatIsIt:
      "Hashing converts a key into a table index so we can store and find values quickly. In interviews it is the fastest way to trade memory for speed.",
    concept: `
...
Answer: multiple keys end up in the same bucket, so lookups take longer.`,
  ...
  7: {
    title: "Binary Trees",
    whatIsIt:
      "A binary tree is a hierarchical structure where each node has at most two children. Traversals are the main skill: preorder, inorder, postorder, and level order.",
    concept: `
      1
     / \
    2   3
...
Answer: only BSTs guarantee sorted inorder.`,
  ...
  8: {
    ...
    whatIsIt:
      "A binary search tree keeps left subtree values smaller and right subtree values larger than the root. That ordering gives fast search, insert, and delete when balanced.",
    concept: `
      8
     / \
    3   10
...
Answer: every comparison cuts the search space roughly in half.`,
  ...
  9: {
    ...
    whatIsIt:
      "A heap is a complete binary tree with a priority rule. Min-heaps and max-heaps are used whenever you need quick access to the smallest or largest item.",
    concept: `
Min-heap example:
      1
     / \
...
Answer: top is stored at the root, insert may need to bubble up.`,
  ...
  10: {
    title: "Tries",
    whatIsIt:
      "A trie stores strings by prefix. It is the right tool for autocomplete, dictionary lookup, and prefix-based search.",
    concept: `
root
 ├─ c ─ a ─ t
 └─ c ─ a ─ r
...
Answer: once the prefix node is found, the remaining subtree holds all completions.`,
  ...
  11: {
    title: "Graphs Basics",
    whatIsIt:
      "A graph models relationships between nodes. Use graphs for networks, dependencies, routes, schedules, and anything that is not naturally linear.",
    concept: `
...
Answer: it explores edges layer by layer, so the first time you reach a node is the shortest distance.`,
  ...
  12: {
    ...
    whatIsIt:
      "Sorting arranges elements into a chosen order. It is often the cheapest way to make a hard problem easier because sorted order enables two pointers and binary search.",
    concept: `
Unsorted: [5, 1, 4, 2, 8]
Sorted:   [1, 2, 4, 5, 8]
...
Answer: because a sorted structure enables faster downstream patterns like two pointers and binary search.`,
  ...
  13: {
    title: "Binary Search",
    whatIsIt:
      "Binary search finds an answer by repeatedly halving the search space. It works on sorted data or on any monotonic predicate.",
    concept: `
Array: [1, 3, 5, 7, 9, 11]
Target: 7
...
Answer: it must be monotonic.`,
  ...
  14: {
    ...
    whatIsIt:
      "Recursion solves a problem by calling the same function on smaller inputs until it reaches a base case. It is the foundation of tree and backtracking solutions.",
    concept: `
function(n)
  -> function(n-1)
    -> function(n-2)
...
Answer: it recurses forever until stack overflow.`,
  ...
  15: {
    ...
    whatIsIt:
      "Backtracking explores choices, and when a path fails it undoes the choice and tries the next one. It is exhaustive search with pruning.",
    concept: `
Decision tree:
- choose
- explore
...
Answer: to restore the state before trying the next option.`,
  ...
  16: {
    ...
    whatIsIt:
      "Greedy algorithms make the best local choice at each step hoping it leads to a global optimum. The hard part is proving that the greedy choice is safe.",
    concept: `
At each step:
...
Answer: because a locally best choice may block a better global solution later.`,
  ...
  17: {
    ...
    whatIsIt:
      "Dynamic programming stores answers to overlapping subproblems so each state is solved once. It is the cure for repeated recursion.",
    concept: `
Think in states:
- define state
- define transition
...
Answer: overlapping subproblems plus an optimal substructure.`,
  ...
  18: {
    ...
    whatIsIt:
      "DSU, segment trees, and BIT are advanced tools for fast connectivity and range queries. DSU handles merging sets, while segment trees and BIT answer updates and queries efficiently.",
    concept: `
...
Answer: when you only need prefix/range-sum style queries with point updates.`,
  ...
  19: {
    ...
    whatIsIt:
      "Bit manipulation uses binary representations directly. It is a compact way to toggle flags, test parity, and solve problems with XOR or bit masks.",
    concept: `
Binary examples:
5 = 0101
3 = 0011
...
Answer: it flips the rightmost 1 to 0 and leaves the higher bits untouched.`,
  ...
  20: {
    ...
    whatIsIt:
      "Advanced graph algorithms solve shortest path, minimum spanning tree, and dependency ordering problems. These are the interview-level graph topics.",
    concept: `
Common tools:
