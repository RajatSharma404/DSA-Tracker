...
export default function RoadmapGraph() {
  ...
  const initData = async () => {
    try {
      ...
      const allTopicProblemsPromises = topics.map(topic => dsaApi.getTopicProblems(topic.id));
      ...
      topics.forEach((topic, index) => {
        ...
        if (index > 0) {
          initialEdges.push({
            ...
            style: { stroke: topics[index-1].progressPercentage === 100 ? '#10b981' : '#3b82f6', strokeWidth: 3 },
            markerEnd: { type: MarkerType.ArrowClosed, color: topics[index-1].progressPercentage === 100 ? '#10b981' : '#3b82f6' }
          });
        }
