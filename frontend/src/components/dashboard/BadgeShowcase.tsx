    ...
    <div className="p-6 rounded-[2.5rem] bg-[#0d0d0d] border border-white/5 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-black text-white tracking-tight uppercase flex items-center gap-2">
          ...
          Badges
          <span className="text-[9px] font-bold text-gray-600 ml-1">{unlocked}/{total}</span>
        </h3>
        <Link
          href="/achievements"
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {unlockedBadges.map(b => (
          <div
            key={b.id}
            className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-lg hover:scale-110 transition-transform cursor-default"
            title={b.name}
          >
            {b.icon}
          </div>
        ...
        {unlocked > 6 && (
          <span className="text-[10px] font-black text-gray-500 ml-1">+{unlocked - 6}</span>
        )}
        {unlocked === 0 && (
          <p className="text-[11px] text-gray-600 italic">Solve your first problem to earn a badge!</p>
        )}
      </div>
      ...
      {nextLocked && (
        <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-2">
          <span className="text-sm grayscale opacity-40">{nextLocked.icon}</span>
          <span className="text-[9px] text-gray-600 font-medium">
            Next: <span className="text-gray-400 font-bold">{nextLocked.name}</span>
          </span>
        </div>
      )}
    </div>
