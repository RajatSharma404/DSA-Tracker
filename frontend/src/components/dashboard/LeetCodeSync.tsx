...
export default function LeetCodeSync({ onSyncComplete }: LeetCodeSyncProps) {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (settings?.leetcodeUsername) {
          setUsername(settings.leetcodeUsername);
        }
      } catch (error) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setLastSyncResult({
        ...
        problems: result.syncedProblems,
      });
      setSyncMeta({
        source: result.syncSource,
      ...
      console.error(error);
      alert(
        "Failed to sync with LeetCode. Make sure your username is correct and public.",
      );
    } finally {
      setIsSyncing(false);
    }
      ...
      {lastSyncResult && lastSyncResult.count > 0 && (
        <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-2xl animate-in slide-in-from-bottom-2">
          <p className="text-xs font-bold text-green-400 mb-2 flex items-center gap-2">
            ...
            {syncMeta?.source ? ` (${syncMeta.source})` : ""}
          </p>
          <div className="flex flex-wrap gap-2">
            {lastSyncResult.problems.map((p, i) => (
              <span
                key={i}
                className="text-[10px] bg-green-500/10 text-green-400 px-2 py-0.5 rounded border border-green-500/10"
              >
                {p}
              </span>
            ))}
